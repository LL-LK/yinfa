...
Page({
  ...
  loadTransportModes: function () {
    ...
    api.request({
      url: '/products?category=transport',
      method: 'GET'
    }).then((products) => {
      if (products && products.length > 0) {
        const modes = products.map(p => ({
          id: p.id,
          name: p.name,
          icon: _transportIcon(p.name),
          desc: p.description || '桂林出行方式，方便快捷',
          tips: p.safety_tips || '出行请注意安全',
          suitable: p.tag_name || '推荐'
        }))
        this.setData({
          modes,
          tips: [
...
function buildModes() {
  return [
    ...
    },
    {
      ...
      tips: '可通过滴滴出行、高德地图叫车。市内到阳朔约80-100元。',
      suitable: '最省心舒适'
    },
    {
      id: 3,
      ...
      tips: '桂林至阳朔约1小时，票价25元；桂林至龙脊约2.5小时，票价40元。',
      suitable: '长途景点首选'
    },
    {
      id: 4,
    ...
    },
    {
      ...
      tips: '象鼻山、两江四湖等市区景点可步行游览，景区内观光车10-30元不等。',
      suitable: '休闲慢游'
    }
  ]
}
